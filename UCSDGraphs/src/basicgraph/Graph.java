...
/** An abstract class that implements a directed graph. The graph may have self-loops, parallel edges. 
 * Vertices are labeled by integers 0 .. n-1 and may also have String labels. The edges of the graph are not labeled.
...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for(int v=0; v<numVertices; ++v ) {
			...
			int nrOfNeighbors=0;
			inNeighbors = getInNeighbors(v);
			outNeighbors = getNeighbors(v);
			
			nrOfNeighbors += inNeighbors.size();
			nrOfNeighbors += outNeighbors.size();
			
			degreeSequence.add(nrOfNeighbors);
		}
	...
	public static void main (String[] args) {
		...
		GraphLoader.createIntersectionsFile("data/maps/vasastan.map", "data/intersections/vasastan.intersections");
		...
		GraphAdjList graphFromFile = new GraphAdjList(); // GraphAdjList();
		//GraphLoader.loadRoadMap("data/maps/vasastan.map", graphFromFile);
