 ...
 * 
 * @author sohof dastmard and UCSD MOOC development
 *
 */
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		// We could just square to the adjMatrix so we get all two hop neighbors from all vertices
		//at once. But in this case we will just calculate the appropriate vector of that matrix
		// by taking the dot product of row vector v from ajdMatrix with all the column vectors of ajdMatrix
		// the resulting vector is the two hop neighbors from vertex v.
		...
		for (int i = 0; i < getNumVertices(); i++) { 
			...
			for (int j=0; j< getNumVertices(); j++) {
				...
				//row v is fixed.. as we step through the columns j of left matrix.. in the right matrix
				// column i is fixed as we step trough its rows j. effectively we get the dot product. 
				// of row v from left matrix with column i of righ matrix
			}
			// if calculated dot product row v and column is not zero it means there is a way
			// two reach vertex i from vertex v in two hops. 
			if (res != 0) {
